...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    affine2_dout, grads['W2'], grads['b2'] = affine_backward(dout, affine2_cache)
    relu1_dout = relu_backward(affine2_dout, relu1_cache)
    affine1_dout, grads['W1'], grads['b1'] = affine_backward(relu1_dout, affine1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    # number of hidden layers + 1 output layer
    dims = [input_dim] + hidden_dims + [num_classes]
    for k, (dim_i, dim_j) in enumerate(zip(dims[:-1], dims[1:])):
      w = weight_scale * np.random.randn(dim_i, dim_j)
      b = np.zeros(dim_j)
  ...
  def loss(self, X, y=None):
    ...
    for i in reversed(xrange(self.num_layers)):
      ...
      loss_reg += np.sum(self.params[w_key] ** 2)
