...
def rnn_step_backward(dnext_h, cache):
  ...
  dhraw = (1 - next_h ** 2) * dnext_h # propagate into tanh: (N, H)

  # dx = np.tile(Wx.sum(axis=1, keepdims=True).T, (N, 1))
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  th = h0
  for i in range(T):
    th, tcache = rnn_step_forward(x[:, i, :], th, Wx, Wh, b)
    h[:, i, :] = th
    cache.append(tcache)
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  next_c = np.zeros((N, H))
  for i in range(T):
    next_h, next_c, cac = lstm_step_forward(x[:, i, :], next_h, next_c, Wx, Wh, b)
    cache.append(cac)
    h[:, i, :] = next_h
  ##############################################################################
  #                               END OF YOUR CODE                             #
